...
static const double MUTATION_DEVIATION  = 0.6;
...
double random_in_range(double start, double end){
  ...
  double dn=end-start;
  //give a 10%+0.05 extra in both side, allow new geno to be created
  start=start-dn*0.1-0.05;
  end=end+dn*0.1+0.05;
  double result= start + ((double)rand()/(double)RAND_MAX)*dn;
  return result;
}
...
Genotype genotype_create() {
  ...
  for (i = 0; i < genotype_size; i++)
    gen->genes[i] = random_in_range(-0.9, 0.9);
...
Genotype genotype_crossover2(Genotype parent1, Genotype parent2) {
  
  Genotype child = genotype_create();
  int i;
  for (i=0;i<genotype_size;i++){
    double start=parent1->genes[i];
    double end=parent2->genes[i];
    child->genes[i]=random_in_range(start,end);
  }
